...
public class MemberController extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String fname = jso.getString("firstname");
        String lname = jso.getString("lastname");
        String birthdaystring = jso.getString("birthday");
        /**把birthday從String轉到Date*/
        SimpleDateFormat birthdaysdf = new SimpleDateFormat("yyyy-MM-dd");
        Date birthday = null;
		try {
			birthday = birthdaysdf.parse(birthdaystring);
        ...
        Member m = null;
		try {
			m = new Member(email, password, fname,lname,birthday);
		} catch (ParseException e) {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (id.isEmpty()) {
        	if(email.isEmpty()) {
        		...
        		/** 透過MemberHelper物件的getByEmail()方法自資料庫取回該名會員之資料，回傳之資料為JSONObject物件 */
                ...
                /** 透過JsonReader物件回傳到前端（以JSONObject方式） */
                jsr.response(resp, response);
                System.out.print("ch_3 get的 " + resp);
        	}
        }
        else {
    ...
    public void doPut(HttpServletRequest request, HttpServletResponse response)
        ...
        String fname = jso.getString("firstname");
        String lname = jso.getString("lastname");
        String dob = jso.getString("birthday");
        
        /** 透過傳入之參數，新建一個以這些參數之會員Member物件 */
        Member m = null;
		try {
			//利用simpleDateFormat 設定日期格式後，再將dob轉成字串
			SimpleDateFormat birthdaysdf = new SimpleDateFormat("yyyy-MM-dd");
			Date birthday =  birthdaysdf.parse(dob);
			m = new Member(id, email, password, fname, lname,birthday);
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
